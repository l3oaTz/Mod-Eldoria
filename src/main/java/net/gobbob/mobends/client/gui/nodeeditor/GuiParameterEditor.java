...
public class GuiParameterEditor
{
	...
	public void display(int mouseX, int mouseY, float partialTicks)
	{
		...
		if (changedState != null)
		{
			...
			fontRenderer.drawStringWithShadow(title, x + (width - fontRenderer.getStringWidth(title)) / 2, y + 3,
					0xffffff);
		}
	...
	public void select(GuiAnimationNode node)
	{
		...
		radioField.enable();
		radioField.setButton(0, 64, 12, 12).setElement(81, 0, 10, 10).setNumberOfElements(3).setOffset(2, 2)
				.setPadding(0, 3).setBackground(0, 48, 42, 16);
		radioField2.enable();
	...
	public void select(GuiCalculation calculation)
	{
		...
		radioField.enable();
		radioField.setButton(51, 0, 10, 10).setElement(21, 6, 6, 6).setNumberOfElements(5).setOffset(2, 2)
				.setPadding(0, 4).setBackground(51, 10, 57, 14);
		radioField2.disable();
	...
	public boolean mouseClicked(int mouseX, int mouseY, int event)
	{
		...
		{
			selectedNode.setModifier(modifierList.getSelectedValue() == null ? null
					: EnumModifier.values()[(Integer) modifierList.getSelectedValue()]);
		} else
		{
