...
public class GuiRadio {
	...
	/*
	 * Used to indicate which operator is selected/hovered over.
	 * The value of -1 represents that nothing is hovered over.
	 * Values between 0 and (n-1) represent different operators
	 * */
	...
	public void display() {
		...
		for(int i = 0; i < numberOfElements; i++) {
			if(i == selectedId) {
				Draw.texturedModalRect(x+offsetX+i*buttonWidth, y+offsetY,
			...
			Draw.texturedModalRect(x+i*buttonWidth+elementOffset, y+elementOffset, elementX+i*elementWidth, elementY, elementWidth, elementHeight);
		}
	}
	...
	public boolean areCoordinatesInBounds(int cX, int cY) {
		return cX >= x+offsetX && cX < x+bgWidth &&
			   cY >= y+offsetY && cY < y+bgHeight;
	}
