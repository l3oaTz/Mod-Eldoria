...
public class GuiDropDownList {
	...
	public GuiDropDownList addEntry(String displayName) {
		entries.add(new Entry(entries.size(), displayName, entries.size()));
		updateDimensions();
		return this;
	}
	
	public GuiDropDownList addEntry(String displayName, Object value) {
		entries.add(new Entry(entries.size(), displayName, value));
		updateDimensions();
		return this;
	}
	...
	public void update(int mouseX, int mouseY) {
		...
		else if(mouseX >= x && mouseX <= x+getWidth() &&
		   mouseY >= y+HEIGHT && mouseY <= y+getVisualHeight()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int event) {
		...
		return mouseX >= x && mouseX <= x+width &&
			   mouseY >= y && mouseY <= y+getVisualHeight();
	}
	...
	public void display() {
		...
		Draw.rectangle(this.x+1, this.y+1, this.width-2, this.HEIGHT-2, hovered||dropped ? 0xff222222 : -16777216);
		GlStateManager.color(1, 1, 1, 1);
		
		boolean noValue = noValueAllowed && selectedId == 0;
		String text = noValue ? "None" : this.fontRenderer.trimStringToWidth(getSelectedEntry().getDisplayName(), this.getWidth()-20);
		this.fontRenderer.drawStringWithShadow(text, x+5, y+4, noValue ? 0x999999 : 0xe2e2e2);
		
		Draw.rectangle_xgradient(x+width-40, y+1, 27, HEIGHT-2, 0x00000000, hovered||dropped ? 0xff222222 : -16777216);
		...
		if(dropped) {
			...
			for(int i = 0; i < elements; i++) {
				...
				String name = noValue ? "None" : this.fontRenderer.trimStringToWidth(entries.get(entryID).getDisplayName(), getWidth());
				this.fontRenderer.drawStringWithShadow(name, x+3, y+HEIGHT+i*ELEMENT_HEIGHT+2, noValue ? 0x999999 : 0xe2e2e2);
			}
			
			if(showScrollBar()){
				Draw.rectangle(x+width-SCROLLBAR_WIDTH-1, y+HEIGHT, SCROLLBAR_WIDTH, elements*ELEMENT_HEIGHT+2, 0xff222222);
				Draw.rectangle(x+width-SCROLLBAR_WIDTH-1, y+HEIGHT+scrollBarY, SCROLLBAR_WIDTH, scrollBarHeight, scrollBarHovered ? 0xff666666 : 0xff444444);
			}
		}
	}
	...
	public Entry getSelectedEntry() {
		if(selectedId < 0 || selectedId >= entries.size() + (noValueAllowed ? 1 : 0))
			return null;
		if(noValueAllowed && selectedId == 0) return null;
	...
	public Object getSelectedValue() {
		if(getSelectedEntry() == null) return null;
		return getSelectedEntry().getValue();
	}
	
	public void updateDimensions() {
		scrollBarHeight = (int) ((float)getNumberOfElements()/entries.size()*(getNumberOfElements()*ELEMENT_HEIGHT+2));
		scrollBarY = (int) (getScrollPercentage()*(getNumberOfElements()*ELEMENT_HEIGHT+2-scrollBarHeight));
	}
